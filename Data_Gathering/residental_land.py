...
headers = {
    ...
    'referer' : f'https://www.99acres.com/residential-land-in-{City}-ffid', # is used to specify the URL of the page that linked to the resource being requested
...
if os.path.exists("last_page_land.txt"):
    with open("last_page_land.txt", "r") as f:
        start = int(f.read())
...
try:
        while page_no < end:
                ...
                url = f'https://www.99acres.com/residential-land-in-{City}-ffid?page={page_no}'
                ...
                for soup in pagesoup.select_one('div[data-label="SEARCH"]').select('section[data-hydration-on-demand="true"]'):
                        ...
                        try:
                                ...
                                price_per_sqft = ''
                                if (div := soup.find("div", class_="resBuy__area2Type ellipsis")):
                                        
                                        price_per_sqft = div.get_text(strip=True)

                                elif(div := soup.find("div",class_="resBuy__area2Type resBuy__pdTop ellipsis")) :
                                                price_per_sqft = div.get_text(strip=True)

                                elif(div := soup.find("div",class_="resBuy__area2Type resBuy__perSqftWrapCol resBuy__pdTop ellipsis")) :
                                                price_per_sqft = div.get_text(strip=True)
                        ...
                        try:
                                if (P_status := soup.find("div", class_="resBuy__ribbonSticky", testid="RESALE_TAG")):
                                        P_status = P_status.find("div").get_text(strip=True)
                                
                                elif (P_status := soup.find("div", class_="PseudoTupleRevamp__ribbon")):
                                        P_status = P_status.find("div").get_text(strip=True)
                        ...
                        try:
                            ...
                            if not address:
                                if (add := soup2.find("div", class_="cDealer__hide")):
                                    address = add.find("div", class_="body_small").text
                                else:
                        ...
                        elif "Plot area" in project_name :
                        ...
                        try:
                                ...
                                if (table := soup2.find("table", class_="pd__tableWrap")):
                                        if (super_built := table.find("tr", id="Super Area")):
                                                plot_area = super_built.find("div", class_="").get_text(strip=True)
                                        else:
                                                plot_area = ''
                                        if (carpet := table.find("tr", id="Length")):
                                                length = carpet.find("div", class_="").get_text(strip=True)
                                        else:
                                                length = ''
                                        if (carpet := table.find("tr", id="Breadth")):
                                                breadth = carpet.find("div", class_="").get_text(strip=True)
                                        else:
                                                breadth = ''
                                        a_type = "Plot_area " + plot_area + 'l*b :' + length +"*"+ breadth
                        ...
                        try:
                            ...
                            if (div1 := soup2.find_all("div", class_="caption_strong_small_semi")):
                                for div in div1:
                                    ...
                                    if 'floors' in text.lower() or 'floor' in text.lower():
                                        ...
                                        break
                            if (floor == ' '):
                                tr = soup2.find("table", class_="AboutProjectDetail__specificTable")
                                if tr:
                                    for div in tr.find_all('tr'):
                                        text = div.get_text(strip=True)
                                        if 'floors' in text.lower() or 'floor' in text.lower():
                                            floor = text
                                            break
                            if (floor == ' '):
                                ...
                                for line in script:
                                    ...
                                    pattern = r'"totalFloor":( *)"(\d+)"'
                                    if (match := re.search(pattern, txt)):
                                        floor = match.group().split(":")[1].replace('"', '')
                                        break
                                    else:
                                        floor = "not available"
                        except Exception as e:
                        ...
                        try:
                                ...
                                if not age_possession or not completion_date :
                                 if(about_div := soup2.find_all("script")) :
                                               pattern = r'Possession( *):( *)(\w( *))+ \*'
                                               for div in about_div:
                                                        text = div.get_text(strip=True)
                                                        if (match := re.search(pattern, text)):
                                                                age_possession = match.group()
                                                                break
                                                        else:
                        ...
                        # No of open sides :
                        try :
                                if(table := soup2.find("tr",id ="No. of Open Sides")) :
                                      
                                      No_open_side = table.find("div", class_="").get_text(strip = True)
                                else :
                        ...
                        try :
                                if(table := soup2.find("tr", id="Boundary wall")) :
                                      b_wall = table.find("div", class_ = "").get_text(strip = True)
                                else :
                                      b_wall = ''
                        except Exception as e :
                               b_wall = ''
                        ...
                        try :
                                if(table := soup2.find("tr",id="Approved By*")) :
                                    auth_approved = table.find("div",class_="").get_text(strip = True)
                                else :
                                      auth_approved = ""
                        except Exception as e :
                               auth_approved = ''
                        ...
                        try :
                                if(table := soup2.find("tr",id ="Corner Property")) :
                                    Corner_property = table.find("div",class_="").get_text(strip = True)
                                else :
                                      Corner_property = ""
                        except Exception as e :
                               Corner_property = ''
                        ...
                        try :
                                if(table := soup2.find("tr",id ="Gated Community")) :
                                    Gated = table.find("div",class_="").get_text(strip = True)
                                else :
                                      Gated = ""
                        except Exception as e :
                               Gated = ''
                        ...
                        try :
                                if(table := soup2.find("tr", id = "Width of facing road")) :
                                      road = table.find("div",class_ = "").get_text(strip=True)
                                else :
                                       road = ""
                        except Exception as e :
                               road = ''
                        ...
                        try :
                               if(table := soup2.find("tr", id ="Ownership" ))  :
                                      ownership = table.find("div",class_ = "").get_text(strip=True)
                               else : 
                                      ownership = ''
                        except Exception as e :
                               ownership = '' 
                        ...
                        property_data = {
                                ...
                                'No_open_sides' : No_open_side,
                                'Boundary_Wall' : b_wall,
                                'Width of Facing road' : road,
                                "Ownership" : ownership, 
                                "Authority Approved": auth_approved,
                ...
                page_no += 1
                with open("last_page_land.txt", "w") as f:
                        f.write(str(page_no))
                pages_processed += 1
                ...
                if pages_processed % 5 == 0:
                        ...
                        if not Land.empty:
                                if os.path.isfile(csv_file_path):
                                        ...
                                        Land.to_csv(csv_file_path, mode='a', header=True, index=False)
                                Land = pd.DataFrame()
