...
if os.path.exists("last_page_house.txt"):
    with open("last_page_house.txt", "r") as f:
        start = int(f.read())
...
end = 100 # You can change this as needed
...
try:
    while page_no < end:
        ...
        url = f'https://www.99acres.com/independent-house-in-{City}-ffid?page={page_no}'
        ...
        for soup in pagesoup.select_one('div[data-label="SEARCH"]').select('section[data-hydration-on-demand="true"]'):
            ...
            elif "Plot area" in project_name or "Built Up area" in project_name:
            ...
            try:
                ...
                if (table := soup2.find("table", class_="pd__tableWrap")):
                    if (super_area := table.find("tr", id="Super Area")):
                        super_area = super_area.find("div", class_="").get_text(strip=True)
                    else:
                        super_area = ''
                    if (built := table.find("tr", id="Built-up Area")):
                        built = built.find("div", class_="").get_text(strip=True)
                    else:
                        built = ''
                    if (carpet := table.find("tr", id="Carpet Area")):
                        ...
                        carpet = ''
                    a_type = "super_area " + super_area + " Built up area " + built + " Carpet Area " + carpet
            ...
            House = pd.concat([House, temp_df], ignore_index=True)
            count += 1
        ...
        page_no += 1
        with open("last_page_house.txt", "w") as f:
                        f.write(str(page_no))
        pages_processed += 1
